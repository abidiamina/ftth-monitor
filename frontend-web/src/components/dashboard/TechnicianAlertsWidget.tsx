...
export const TechnicianAlertsWidget = ({ interventions, technicians }: TechnicianAlertsWidgetProps) => {
  const alerts = useMemo(() => {
    ...
    return { delays, overloads, criticalIncidents, total: delays + overloads + criticalIncidents }
      ...
      <div className='stat-pill bg-white/40 relative'>
        ...
        <div className='flex items-center gap-2 mt-2'>
          <p className='text-3xl font-black text-slate-950'>
            0 <span className='text-lg font-bold text-slate-400'>alerte</span>
          </p>
        </div>
      </div>
    )
    ...
    <div className='stat-pill bg-rose-50 border border-rose-200/50 shadow-[0_0_20px_rgba(244,63,94,0.15)] group relative'>
      ...
      <div className='flex items-center gap-2 mt-2'>
        <p className='text-3xl font-black text-rose-950'>
          {alerts.total} <span className='text-lg font-bold text-rose-400/80'>critique(s)</span>
        </p>
      </div>
      ...
      <div className='absolute top-full mt-2 left-0 w-64 bg-slate-900 rounded-2xl p-4 shadow-xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all z-50 pointer-events-none border border-slate-700'>
        <p className='text-slate-300 text-[10px] font-black uppercase tracking-widest mb-3 border-b border-slate-700 pb-2'>
          Details des alertes
        </p>
        {alerts.criticalIncidents > 0 && (
          <p className='text-red-400 text-xs font-bold mb-1'>- {alerts.criticalIncidents} incident(s) critique(s)</p>
        )}
        {alerts.delays > 0 && <p className='text-rose-400 text-xs font-bold mb-1'>- {alerts.delays} mission(s) en retard</p>}
        {alerts.overloads > 0 && (
          <p className='text-amber-400 text-xs font-bold'>- {alerts.overloads} technicien(s) en surcharge (&ge; 3)</p>
        )}
      </div>
    </div>
  )
